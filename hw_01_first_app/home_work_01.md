...
<b style="color: #F00000; border: 2px solid #6B0000; display: inline-block; padding: 10px; margin: 0 10px 0 0;"> NB ! </b> По совету Вячеслава Бандыло <m style="color: firebrick;">ЛУЧШЕ СРАЗУ создавать Django-prjoect, а НЕ Python-project. 
Потому что тогда придется ВРУЧНУЮ прописывать путь к файлу настроек в настройках Django, чтобы 
отображались все связи и зависимости.</m>  
<br>И ничего, что модуль с настройками проекта теперь будет называться как сам проект, главное, чтобы работало нормально.  
